...
function daysTemplate() {
  ...
  const daysData = {
    different_days: daysOfWeek.map((day) => {
      let selectedClass = "";
      if (
        day === get_day_class(daysInstance.get_date_value_1()) &&
        day === get_day_class(daysInstance.get_date_value_2())
      ) {
        selectedClass = "same_day";
      } else if (day === get_day_class(daysInstance.get_date_value_1())) {
        selectedClass = "day1";
      } else if (day === get_day_class(daysInstance.get_date_value_2())) {
        selectedClass = "day2";
      }
